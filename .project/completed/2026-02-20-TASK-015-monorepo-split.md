...
## Decisão de cancelamento — 2026-02-20

Avaliado ao final da entrega do AIPIM 2.0. Nenhum dos gatilhos foi atingido:

- **Bundle npm:** 107 KB (tarball) / 328 KB descompactado — muito abaixo do limiar de 10 MB.
- **Demanda por pacotes independentes:** não existe (nenhum issue, nenhum caso de uso externo).
- **Time externo por área:** projeto solo/pequeno time.

Complexidade de manutenção de monorepo (pnpm workspaces, build ordering, cross-package imports) não se justifica neste momento. Reavaliar apenas se o bundle ultrapassar 10 MB ou houver demanda concreta por `@aipim/core` ou `@aipim/ui` standalone.
