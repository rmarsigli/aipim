...
describe('MCP tool registry', () => {
    it('CORE_TOOLS is an array', async () => {
        const { CORE_TOOLS } = await import('../../src/mcp/tools/index.js')
...
describe('MCP server HTTP endpoint', () => {
        ...
        app.post('/mcp', async (c) => {
            ...
            if (body.method === 'tools/list') {
                return c.json({
                    jsonrpc: '2.0',
                    id: body.id,
                    result: { tools: CORE_TOOLS.map((t) => t.schema) },
                })
            }
    ...
    it('responds to tools/list method', async () => {
        ...
        expect(json.result.tools.length).toBe(CORE_TOOLS.length)
    })
    ...
    it('returns error for unknown tool in tools/call', async () => {
        ...
        app.post('/mcp', async (c) => {
            ...
            if (body.method === 'tools/call') {
                ...
                const tool = CORE_TOOLS.find((t) => t.schema.name === toolName)
                if (!tool) {
