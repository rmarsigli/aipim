...
describe('complete_task', () => {
    ...
    it('throws when task does not exist', () => {
        expect(() => tool.handler(ctx, { taskId: 'TASK-999' })).toThrow('TASK-999 not found')
    })

    it('marks task as done in SQLite', () => {
        seedTask(db, 'TASK-001')
        tool.handler(ctx, { taskId: 'TASK-001' })
        const updated = getTask(db, 'TASK-001')
        expect(updated?.status).toBe('done')
    })

    it('returns success with completedAt', () => {
        seedTask(db, 'TASK-001')
        const result = tool.handler(ctx, { taskId: 'TASK-001' }) as Record<string, unknown>
        expect(result.success).toBe(true)
        expect(result.taskId).toBe('TASK-001')
        expect(typeof result.completedAt).toBe('string')
    })

    it('moves .md file to completed/ when it exists', () => {
    ...
    it('returns fileMoved=null when task has no file', () => {
        const evt: TaskCreatedEvent = {
            id: 'e1',
            type: 'task.created',
            timestamp: new Date().toISOString(),
        ...
        const result = tool.handler(ctx, { taskId: 'TASK-002' }) as Record<string, unknown>
        expect(result.fileMoved).toBeNull()
    })

    it('appends event to events.jsonl', () => {
        seedTask(db, 'TASK-001')
        tool.handler(ctx, { taskId: 'TASK-001', notes: 'All done', actualHours: 3 })
...
describe('update_task_status', () => {
    ...
    it('throws when task does not exist', () => {
        expect(() => tool.handler(ctx, { taskId: 'TASK-999', status: 'in-progress' })).toThrow('TASK-999 not found')
    })

    it('updates task status', () => {
        seedTask(db, 'TASK-001')
        tool.handler(ctx, { taskId: 'TASK-001', status: 'in-progress' })
        expect(getTask(db, 'TASK-001')?.status).toBe('in-progress')
    })

    it('returns from/to', () => {
        seedTask(db, 'TASK-001')
        const result = tool.handler(ctx, { taskId: 'TASK-001', status: 'blocked' }) as Record<string, unknown>
        expect(result.from).toBe('backlog')
        expect(result.to).toBe('blocked')
    })

    it('adds reason as comment when provided', () => {
        seedTask(db, 'TASK-001')
        tool.handler(ctx, { taskId: 'TASK-001', status: 'blocked', reason: 'Waiting for API' })
    ...
    it('does not add comment when reason is absent', () => {
        seedTask(db, 'TASK-001')
        tool.handler(ctx, { taskId: 'TASK-001', status: 'review' })
...
describe('add_comment', () => {
    ...
    it('throws when task does not exist', () => {
        expect(() => tool.handler(ctx, { taskId: 'TASK-999', text: 'hello' })).toThrow('TASK-999 not found')
    })

    it('inserts comment into SQLite', () => {
        seedTask(db, 'TASK-001')
        tool.handler(ctx, { taskId: 'TASK-001', text: 'Looks good!' })
        const rows = db.prepare('SELECT * FROM comments WHERE task_id = ?').all('TASK-001') as Array<{ text: string }>
        expect(rows.length).toBe(1)
        expect(rows[0].text).toBe('Looks good!')
    })

    it('returns commentId and timestamp', () => {
        seedTask(db, 'TASK-001')
        const result = tool.handler(ctx, { taskId: 'TASK-001', text: 'Done' }) as Record<string, unknown>
        expect(result.success).toBe(true)
...
describe('log_decision', () => {
    ...
    it('creates ADR file in decisions/', () => {
        const result = tool.handler(ctx, { title: 'Use SQLite', rationale: 'Fast and local' }) as Record<string, unknown>
        expect(result.success).toBe(true)
    ...
    it('creates decisions dir if missing', () => {
        rmSync(join(TEST_ROOT, '.project/decisions'), { recursive: true, force: true })
        tool.handler(ctx, { title: 'Pick DB', rationale: 'SQLite is great' })
        expect(existsSync(join(TEST_ROOT, '.project/decisions'))).toBe(true)
    })

    it('inserts decision in SQLite', () => {
        tool.handler(ctx, { title: 'Use Hono', rationale: 'Lightweight' })
    ...
    it('includes taskId in ADR file when provided', () => {
...
describe('create_task', () => {
    ...
    it('creates task file and inserts into SQLite', () => {
        const result = tool.handler(ctx, {
            title: 'Add dark mode',
            taskType: 'feat',
            priority: 'P2-M',
    ...
    it('increments task ID from existing tasks', () => {
        seedTask(db, 'TASK-005')
        const result = tool.handler(ctx, { title: 'Next task', taskType: 'chore', priority: 'P3' }) as Record<string, unknown>
        expect(result.taskId).toBe('TASK-006')
    })

    it('starts from TASK-001 when no tasks exist', () => {
        const result = tool.handler(ctx, { title: 'First task', taskType: 'feat', priority: 'P1-S' }) as Record<string, unknown>
        expect(result.taskId).toBe('TASK-001')
    })

    it('includes description in file when provided', () => {
        const result = tool.handler(ctx, {
            title: 'My task',
            taskType: 'fix',
            priority: 'P2-S',
    ...
    it('all tasks appear in list_tasks', () => {
        tool.handler(ctx, { title: 'Task A', taskType: 'feat', priority: 'P1-S' })
        tool.handler(ctx, { title: 'Task B', taskType: 'fix', priority: 'P2-M' })
